  ...
  return (
    <>
      ...
      >
        ...
        {/* Desktop links */}
        <ul className="hidden min-[861px]:flex items-center gap-[18px] min-[1081px]:gap-7 font-nunito font-bold list-none">
          {LINKS.map(l => (
            <li key={l.href}>
              <a href={l.href}
                className="relative text-[.9rem] min-[1081px]:text-[.98rem] text-tinta transition-colors duration-200 hover:text-azul
        ...
        <a href="tel:+555430667350"
          className="hidden min-[861px]:inline-flex items-center gap-2 whitespace-nowrap bg-azul text-white
        ...
        <button onClick={() => setOpen(o => !o)} aria-expanded={open}
          ...
          className="flex min-[861px]:hidden flex-col gap-[5px] bg-transparent border-0 cursor-pointer p-1">
          {[
      ...
      {open && (
        <div role="dialog" aria-modal="true" aria-label="Menu de navegação"
          className="fixed inset-x-0 top-0 z-[190] bg-white/95 backdrop-blur-[14px] pt-20 pb-8 px-6
            flex flex-col gap-4 shadow-suave min-[861px]:hidden">
          {LINKS.map(l => (
